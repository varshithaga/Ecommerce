...
import { getClientList, deleteClient,ClientData ,ClientFormData} from "./api";
...
const ClientPage: React.FC = () => {
  ...
  const fetchClients = async () => {
    try {
      const userList = await getClientList();
      setUsers(userList);
    } catch (err: unknown) {
  ...
  const onClientAdded = (newUser: ClientData) => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(user =>
        ...
        user.company_address?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        user.gst_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
        user.capax_id.toLowerCase().includes(searchTerm.toLowerCase())||
  ...
  return (
    <>
      <PageMeta title="Client" description="Manage client" />
            ...
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                ...
                <TableCell
                  isHeader
                  className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                  onClick={() => handleSort('company_address')}
                >
                  <div className="flex items-center gap-2">
                    Address
                    <span className="text-gray-300 dark:text-gray-600">
                      {sortField === 'company_address' ? (
                        sortDirection === 'asc' ? '↑' : '↓'
                      ) : '↕'}
                    </span>
                  </div>
...
\
      ...
      {totalPages > 1 && (
        ...
        {isEditModalOpen && editClientId !== null && (
        <EditClient
